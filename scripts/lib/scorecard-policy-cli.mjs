import { appendFile, readFile, writeFile } from 'node:fs/promises';
import { resolve } from 'node:path';
import {
  evaluateScorecardPolicy,
  parseEvaluationDate,
  renderSummary
} from './scorecard-policy.mjs';

const root = resolve(import.meta.dirname, '../..');

export async function runScorecardPolicyCli(
  argv = process.argv.slice(2),
  environment = process.env,
  output = console
) {
  const exactArgument = argv[0] ?? 'results.json';
  const policyArgument = argv[1] ?? '.github/scorecard-policy.json';
  const reportArgument = argv[2] ?? 'scorecard-policy-report.json';
  const profileName =
    argv[3] ?? environment.SCORECARD_POLICY_PROFILE ?? 'repository';
  const reportPath = resolve(root, reportArgument);

  let report;
  let evaluationError;
  try {
    const [exactDocument, policy] = await Promise.all([
      readJson(resolve(root, exactArgument), 'Scorecard exact JSON'),
      readJson(resolve(root, policyArgument), 'Scorecard policy')
    ]);
    report = evaluateScorecardPolicy({
      exactDocument,
      policy,
      profileName,
      evaluatedAt: parseEvaluationDate(environment.SCORECARD_POLICY_DATE),
      exactScorecardFile: exactArgument,
      policyFile: policyArgument
    });
  } catch (error) {
    evaluationError = error;
    const evaluatedAt = safeEvaluationTimestamp(environment.SCORECARD_POLICY_DATE);
    report = {
      schemaVersion: 5,
      evaluatedAt,
      profile: profileName,
      exactScorecardFile: exactArgument,
      policyFile: policyArgument,
      scorecardVersion: null,
      scorecardCommit: null,
      repository: null,
      repositoryCommit: null,
      passed: false,
      failures: [`Scorecard policy evaluation failed: ${error.message}`],
      checks: [],
      error: {
        name: error.name,
        message: error.message
      }
    };
  }

  await writeFile(reportPath, `${JSON.stringify(report, null, 2)}\n`);

  const summary = renderSummary(report);
  if (report.passed) {
    output.log(summary);
  } else {
    output.error(summary);
  }
  if (environment.GITHUB_STEP_SUMMARY) {
    await appendFile(environment.GITHUB_STEP_SUMMARY, `${summary}\n`);
  }

  return {
    exitCode: report.passed ? 0 : 1,
    report,
    error: evaluationError
  };
}

async function readJson(path, label) {
  let text;
  try {
    text = await readFile(path, 'utf8');
  } catch (error) {
    throw new Error(`${label} file could not be read: ${error.message}`);
  }
  try {
    return JSON.parse(text);
  } catch (error) {
    throw new Error(`${label} file is not valid JSON: ${error.message}`);
  }
}

function safeEvaluationTimestamp(value) {
  if (value) {
    const parsed = new Date(value);
    if (!Number.isNaN(parsed.getTime())) {
      return parsed.toISOString();
    }
  }
  return new Date().toISOString();
}
