...
const policy = JSON.parse(
  readFileSync(join(root, '.github/repository-policy.json'), 'utf8')
);
const modulePromise = import(
  pathToFileURL(join(root, 'scripts/lib/repository-audit.mjs')).href
);
...
test('repository settings that are not visible fail closed', async () => {
  const { auditRepositorySnapshot } = await modulePromise;
  const snapshot = makeSnapshot();
  delete snapshot.metadata.allow_squash_merge;
  const { failures } = auditRepositorySnapshot(snapshot, policy);
  assert.ok(
    failures.some(item =>
      item.includes('Required repository setting allow_squash_merge is not visible')
    )
  );
});

test('ruleset bypass actors that are not visible fail closed', async () => {
  const { auditRepositorySnapshot } = await modulePromise;
  const snapshot = makeSnapshot();
  delete snapshot.rulesetDetail.bypass_actors;
  const { failures } = auditRepositorySnapshot(snapshot, policy);
  assert.ok(
    failures.some(item => item.includes('Ruleset bypass actors are not visible'))
  );
});
...
test('missing relevant rule parameters fail closed', async () => {
  ...
  const failures = await evaluate(rules);
  assert.ok(
    failures.some(item => item.includes('parameters are not visible or invalid'))
  );
});
...
test('malformed effective rules fail closed', async () => {
  ...
  assert.ok(
    failures.includes('Active branch rules contain an invalid rule entry.')
  );
});
  ...
  const client = createGitHubApiClient({
    ...
    fetchImpl: async (url, options) => {
      ...
      return new Response('', {
        status: 301,
        headers: {
          location: 'https://api.github.test/repos/new/name'
        }
      });
    }
  });
...
test('custom GITHUB_API_URL paths are preserved', async () => {
  ...
  assert.equal(
    observedUrl,
    'https://ghe.example.test/api/v3/repos/owner/repo'
  );
});
...
test('the repository-owned ruleset rejects unexpected status checks while effective rules may be stricter', async () => {
  ...
  assert.ok(
    failures.includes(
      'Repository ruleset contains unexpected required status check unexpected.'
    )
  );
});

test('repository policy arrays and malformed status-check values fail fast', async () => {
  const { validatePolicy } = await modulePromise;
  const repositoryArray = structuredClone(policy);
  repositoryArray.repository = [];
  assert.throws(
    () => validatePolicy(repositoryArray),
    /repository settings as an object/
  );

  const malformedChecks = structuredClone(policy);
  malformedChecks.requiredStatusChecks = ['Required', { context: 'Fuzz' }];
  assert.throws(
    () => validatePolicy(malformedChecks),
    /unique, non-empty requiredStatusChecks strings/
  );
});
...
function makeSnapshot() {
  ...
  return {
    ...
    vulnerabilityReporting: {
      enabled: policy.privateVulnerabilityReporting
    },
    rulesets: [
      {
        id: 1,
