...
| Security testing | Repository-owned CodeQL and event-driven advanced CodeQL, Gitleaks, dependency review, npm audit, token/URL controls, and Scorecard policy | `SAST`, `CodeQL`, `Gitleaks`, dependency and Scorecard checks |
...
| Continuous testing / QAOps | Pull-request, protected-branch, and manual triggers execute the appropriate risk-based suites | GitHub Actions and protected required checks |
...
Extended QA runs only through manual dispatch. Property-based fuzzing remains a pull-request and push gate; manual dispatch supports a larger generated-case budget. Recurring schedules are disabled to conserve GitHub Actions resources. Ordinary CI runs on code changes or manual dispatch.
...
Every pull request publishes machine-readable JUnit, LCOV, CodeQL SARIF, performance, smoke, SBOM, checksum, and platform-compatibility evidence. Actions artifacts, logs, and run history have a three-day retention policy, and caches have a three-day retention limit. Download any evidence needed longer before it expires. Tagged releases additionally publish signed GitHub attestations that bind the released VSIX and SBOM to the protected source revision.
